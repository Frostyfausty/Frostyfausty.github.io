...
This is an EDA Project completed during the REU at Carnegie Mellon University. This was collaboration project with Olivia Kim and Arturo Espana. An Exploratory Data Analysis (EDA) project involves systematically examining a dataset to understand its main characteristics, identify patterns, and potentially uncover insights before formal analysis or modeling. 
...
ggplot(ccd_prop, aes(x = fct_rev(factor(county)), y = proportion, fill = vaccine)) +
  ...
  scale_fill_manual(values = c("pfizer" = "darkblue", 
                               ...
                               "jj" = "#FF9999"))
```
Through this visualization we can understand that Moderna and Pfizer were distributed throughout counties within California at a higher rate in comparison to Johnson & Johnson (JJ). This is a result of several factor such as storage requirement and well as CDC approval dates.
...
```
Within this chart we can see the frequency of delivery rates between such vaccines. The significance of this visualization allows us to understand that consistency of each vaccine's delivery.